...
def process(dir_path, rbsc: RBSC, do_LPF=True, do_curvefit=True):
    ...
    if do_curvefit == True:
        ...
        csv_results_poly3d = []
        csv_results_log = []
        csv_results_poly4d_sigmoid = []
        json_results_poly4d = []
        json_results_poly3d = []
        json_results_log = []
        json_results_poly4d_sigmoid = []
        ...
        for filename in tqdm(sorted_list):
            if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                ...
                coef1, coef2, coef3 = rbsc.postprocess(img_path)
                # coef1, coef2, coef3, coef4 = rbsc.postprocess(img_path)
                if coef1 is not None:
                    csv_results_poly4d.append((filename, coef1))
                    csv_results_poly3d.append((filename, coef2))
                    csv_results_log.append((filename, coef3))
                    # csv_results_poly4d_sigmoid.append((filename, coef4))

                    json_results_poly4d.append({'Image Filename': filename, 'Coefficients': coef1.tolist()})
                    json_results_poly3d.append({'Image Filename': filename, 'Coefficients': coef2.tolist()})
                    json_results_log.append({'Image Filename': filename, 'Coefficients': coef3.tolist()})
                    # json_results_poly4d_sigmoid.append({'Image Filename': filename, 'Coefficients': coef4.tolist()})
        ...
        # df3 = pd.DataFrame(csv_results_poly4d_sigmoid, columns=['Image Filename', 'Coefficients'])
        ...
        # df3.to_csv(result_dir + '/' + 'curve_fit_result-poly4d_sigmoid.csv', index=False)
        ...
        with open(result_dir + '/' + 'curve_fit_result-poly4d_sigmoid.json', 'w') as f:
            json.dump(json_results_poly4d_sigmoid, f)
            print(f".json file saved to {result_dir + '/' + 'curve_fit_result-poly4d_sigmoid.json'}.")
